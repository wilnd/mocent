	...
	if(result !=null && result.length>0){
		...
		for(var i=0;i<result.length;i++){
			...
			if(j==1 || j==2 || j==3 || j==5 || j==6 || j==7 ){
				...
				innerArr+="<div><a href=\"javascript:void(0);\" onclick=\"tofindNewsById("+result[i].id+")\"><img src=\""+result[i].img_path+"\"/></a></div>";
	...
	$.ajax({
		...
		success: function(ret){
			...
			if(obj != null && obj != "0"){
				...
				var publishTimes=publishTime.split("-");
				var timeAndAuthor=publishTimes[0]+"年"+publishTimes[1]+"月"+publishTimes[2]+"日           作者： "+author;
...
function getNewsTitleList(){
	...
	if(result!=null && result.length>0){
		...
		for(var i=0;i<result.length;i++){
			...
			innerArr+="<li><a href=\"javascript:void(0);\" onclick=\"findNewsById("+result[i].id+")\">"+title+"</a></li>";
