...
function updateClw(){
	var html=window.editor.html();
	window.editor.sync();//编辑器同步
	html=$("#editor_id_clwnr").val();//内容
	
	var clwTitle = $("#clwnr").val();//标题
	var img_path = $("#clwtpdz").val();//图片地址
	
	$.ajax({
		type: "POST",
		url: "mocent/updateClw",
		data: { title: clwTitle, content:html,img_path: img_path },
		dataType: "text",
		async: false,
		success: function(data){
			if(data == "1"){
...
function loadNews(){
	...
	if(result != null && result.length > 0)
	{
		...
		gProductGrid = $.fn.bsgrid.init("tabNewsList", {
			localData: result,
...
function id(obj)
{
	var id = gProductGrid.getRecordIndexValue(obj, "id");
...
function title(obj)
{
	var title = gProductGrid.getRecordIndexValue(obj, "title");
...
function author(obj)
{
	var author = gProductGrid.getRecordIndexValue(obj, "author");
...
function state(obj)
{
	var state = gProductGrid.getRecordIndexValue(obj, "state");
	var html ="";
	if(state == 1)
	{
		html="是";
...
function is_top(obj)
{
	var is_top = gProductGrid.getRecordIndexValue(obj, "is_top");
	var html =[];
...
function editor1(obj)
{
	...
	html.push("<button type=\"button\"  onclick=\"tochangeNews("+id+ ")\">修改</button>");
	html.push("</div>");
...
function updateNews(){
	...
	var newsInfo = {
			...
			publish_time: $("#publish_time_u").val(),
			top:mytop,
			effective:myeffective,
